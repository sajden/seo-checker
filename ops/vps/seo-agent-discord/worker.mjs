...
function currentExperimentMeasurementSummary() {
  ...
  return {
    ...
    day14Measured: experiments.filter((item) => item.measurements?.followups?.day14?.evaluation).length,
    day30Measured: experiments.filter((item) => item.measurements?.followups?.day30?.evaluation).length,
    measurementUnavailable: experiments.filter((item) => item.measurementClosedAt).length,
...
async function maybeQueueAutonomousCodeActions(workspaces) {
  ...
  for (const workspace of autonomousWorkspaceOrder(workspaces)) {
    ...
    log('autonomous_code_queued_silently', {
      workspace: workspace.label || workspace.id || null,
      actionId: candidate.action.id,
      targetUrl: candidate.action.targetUrl || null
    })
    saveState()
    return
  }
...
async function buildCodexOpportunityAction(workspace, targetChannelId = null, context = {}) {
  ...
  const prompt = [
    ...
    '{"actions":[{"title":"kort titel","targetUrl":"https://...","keyword":"exakt verifierbar sökfras/fokus","evidenceType":"gsc|crawl|keyword_planner","actionType":"ranking_ctr|ranking_relevance|technical|internal_links|ai_visibility","requiredFields":["title","h1","intro"],"priority":"high|medium","category":"content|internal-links","why":"mätvärden och varför detta är bästa nästa experimentet","hypothesis":"förväntad ranking/CTR-effekt","recommendedAction":"exakt liten copy/metadata/internlänksändring i repo"}]}',
    '',
  ...
  for (const suggestion of suggestions) {
    ...
    const action = {
      ...
      actionType: String(suggestion.actionType || (evidenceCheck.evidence.type === 'gsc' ? 'ranking_relevance' : 'technical')).slice(0, 40),
      requiredFields: Array.isArray(suggestion.requiredFields) ? suggestion.requiredFields.slice(0, 8) : [],
      hypothesis: String(suggestion.hypothesis || '').slice(0, 600),
      primaryQuery: keyword,
      rankingMetrics: evidenceCheck.evidence.type === 'gsc' ? {
        impressions: evidenceCheck.evidence.impressions,
        clicks: evidenceCheck.evidence.clicks,
...
function recordSeoExperiment(action, workspace, targetChannelId, result, meta = {}) {
  ...
  state.seoExperiments[id] = {
    ...
    actionType: action.actionType || (keyword ? 'ranking_relevance' : 'unclassified'),
    primaryQuery: action.primaryQuery || keyword,
...
function formatActionMessage(action, workspacePolicy, workspace, review = null) {
  ...
  const lines = [
    ...
    action.hypothesis ? `Hypotes: ${String(action.hypothesis).slice(0, 360)}` : '',
